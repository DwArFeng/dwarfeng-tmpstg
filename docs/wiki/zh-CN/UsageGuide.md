...
临时存储是 dwarfeng-tmpstg 的核心概念，它是一个可以存储任意二进制数据的容器。
每个临时存储都有一个唯一的键（key），通过这个键可以访问存储的数据。
...
public class ExceptionHandlingShouldAvoid {

    private static final Logger LOGGER = LoggerFactory.getLogger(ExceptionHandlingShouldAvoid.class);
